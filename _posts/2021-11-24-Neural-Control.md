...
In _Neural-Lander_, we use a deep neural network $\hat{g}(x_t,u_t)$ to approximate the residual dynamics $g$. We proved that if the Lipschitz constant of the DNN is smaller than some system-dependent threshold $\gamma$, the learning-based controller is exponentially stable (i.e., the trajectory tracking error exponentially converges to some small error ball whose size is related to the learning performance). Recall that for a function $h(z)$, the Lipschitz constant $\|h\|_{\text{Lip}}$ is defined as the smallest value such that 
$$\forall z,z',\|h(z)-h(z')\|/\|z-z'\| \leq \|h\|_{\text{Lip}}.$$

Basically, the Lipschitz threshold $\|\hat{g}\|_{\text{Lip}} \leq \gamma$ requires some global smoothness property of the DNN $\hat{g}$. Note that this constraint is from control theory (i.e., we must jointly consider learning and control theory to have such a result), and necessary! Actually, modern DNN training itself is far away from satisfying $\|\hat{g}\|_{\text{Lip}} \leq \gamma$: for our drone systems, $\gamma\approx16$, but a 8-layer DNN without Lipschitz constrained training yields $\|\hat{g}\|_{\text{Lip}} > 8000$, which led to a drone crash in our experiments. In practice, we use spectral normalization to ensure that $\|\hat{g}\|_{\text{Lip}} \leq 16$. With the spectrally normalized DNN, we can achieve the following agile manuever super close to the ground:
